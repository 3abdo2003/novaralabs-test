...
export default async function handler(req, res) {
    ...
    if (req.method === 'POST') {
        ...
        try {
            ...
            while (tokens.length < count) {
                // randomBytes is universally supported in all Node versions, unlike randomUUID
                const tokenStr = crypto.randomBytes(16).toString('hex').substring(0, 16).toUpperCase();
    ...
    if (req.method === 'DELETE') {
        ...
        try {
            await db.collection("qr_tokens").deleteMany({ batchId: new ObjectId(batchId) });
            await db.collection("qr_batches").deleteOne({ _id: new ObjectId(batchId) });
            return res.status(200).json({ success: true, message: 'Batch deleted successfully' });
        } catch (error) {
            console.error('Delete Batch Error:', error);
            return res.status(500).json({ success: false, error: 'Internal server error while deleting batch' });
        }
    }
